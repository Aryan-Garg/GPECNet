...
def train(train_dataset):
    ...
    for i, (traj, mask, initial_pos) in enumerate(zip(train_dataset.trajectory_batches, train_dataset.mask_batches, train_dataset.initial_pos_batches)):
        ...
        x = x.to(device)
        dest = y[:, -1, :].to(device)   #(513, 2)
#        print('destination', dest.shape)
        future = y[:, :-1, :].contiguous().view(y.size(0),-1).to(device)   #(513, 22)
#        print('future', future.shape)

        dest_recon, mu, var, interpolated_future, mu_f, var_f = model.forward(x, initial_pos, dest=dest, mask=mask, future = future, device=device)

        optimizer.zero_grad()
        rcl, kld, adl, kld_f = calculate_loss(dest, dest_recon, mu, var, criterion, future, interpolated_future, mu_f, var_f)
        loss = rcl + kld*hyper_params["kld_reg"] + adl*hyper_params["adl_reg"] + kld_f
        loss.backward()
    ...
    with torch.no_grad():
        for i, (traj, mask, initial_pos) in enumerate(zip(test_dataset.trajectory_batches, test_dataset.mask_batches, test_dataset.initial_pos_batches)):
            ...
            for _ in range(best_of_n):
                ...
                dest_recon = model.forward(x, initial_pos, mask=mask, device=device)
            ...
            all_l2_errors_dest = np.array(all_l2_errors_dest)  #(20, 2829)
#            print('all_l2_errors_dest', all_l2_errors_dest.shape)
            
            all_guesses = np.array(all_guesses) #(20, 2829, 2)
#            print('all_guesses', all_guesses.shape)
            
            # average error
            l2error_avg_dest = np.mean(all_l2_errors_dest) 
#            print('l2error_avg_dest', l2error_avg_dest)

            # choosing the best guess
            indices = np.argmin(all_l2_errors_dest, axis = 0)  #(2829)
#            print('indices', indices.shape)

            best_guess_dest = all_guesses[indices,np.arange(x.shape[0]),  :] #(2829, 2)
#            print('best_guess_dest', best_guess_dest.shape)
            
            # taking the minimum error out of all guess
            l2error_dest = np.mean(np.min(all_l2_errors_dest, axis = 0))
#            print('l2error_dest', l2error_dest)
            ...
            for _ in range(best_of_n):
                ...
                interpolated_future = model.predict(x, best_guess_dest, mask, initial_pos)  #(2829, 22)
...
#            indices_f = np.argmin(all_l2_errors_dest_f, axis = 0)  #(2829)
            
#            best_guess_dest_f = all_guesses[indices_f,np.arange(x.shape[0]),  :] #(2829, 2)
            
            l2error_dest_f = np.mean(np.min(all_l2_errors_dest_f, axis = 0))

            # using the best guess for interpolation
            
#            print('interpolated_future', interpolated_future.shape)
...
#            l2error_overall = np.mean(np.linalg.norm(y - predicted_future, axis = 2))
            ...
            l2error_dest_f /= hyper_params["data_scale"]
            ...
            print('Test time error overall (ADE) best: {:0.3f} and mean: {:0.3f}'.format(l2error_overall, l2error_dest_f))

    return l2error_overall, l2error_dest, l2error_avg_dest, l2error_dest_f
...
for e in range(hyper_params['num_epochs']):
    ...
    test_loss, final_point_loss_best, final_point_loss_avg, test_loss_best = test(test_dataset, best_of_n = N)


    if best_test_loss > test_loss_best:
        print("Epoch: ", e)
    ...
    print("Test Min ADE", test_loss_best)
